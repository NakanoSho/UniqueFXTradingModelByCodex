...
class TestGates(unittest.TestCase):
    ...
    def test_trend_gate(self):
        self.assertTrue(gates.trend_gate(True, 1.2, True, True))
        self.assertFalse(gates.trend_gate(True, 1.6, True, True))
        self.assertFalse(gates.trend_gate(False, 1.2, True, True))

    def test_carry_gate_and_crash(self):
        self.assertEqual(gates.carry_gate(True), 0.0)
        self.assertEqual(gates.carry_gate(False), 1.0)
        self.assertTrue(gates.carry_crash_stop(-0.03))
        self.assertFalse(gates.carry_crash_stop(-0.01))

    def test_value_gate(self):
        self.assertEqual(gates.value_gate(0.5, 11.0), 1.0)
        self.assertEqual(gates.value_gate(0.5, 13.0), 0.5)
        self.assertEqual(gates.value_gate(0.5, 15.0), 0.0)
        self.assertEqual(gates.value_gate(1.1, 11.0), 0.0)

    def test_satellite_gates(self):
        self.assertTrue(gates.satellite_m1_gate(True, 0.03, -0.01, True))
        self.assertFalse(gates.satellite_m1_gate(True, 0.05, -0.01, True))
        self.assertTrue(gates.satellite_m2_gate(True, 0.01, 1.0, 1.1))
        self.assertFalse(gates.satellite_m2_gate(True, 0.03, 1.0, 1.1))
