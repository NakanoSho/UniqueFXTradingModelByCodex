...
1. クリーンルーム手順が文書化され、研究・コード・データの出自（provenance）が追跡可能
2. データ（Spot/Forward/金利/ボラ/主要マクロ）の取得→正規化→保存→QCが自動で回る
3. バックテスト用の最小データモデル（OHLC/ミッド/スプレッド/フォワード/金利）が揃い、再現可能
4. 監視（P&L、エクスポ、レバ、コスト、異常約定、DD）の日次レポート＋アラート条件が動く
...
### B1. 推奨最小スタック
- 言語: Python
- 実行: CLI（`make` or `task`）+ OSスケジューラ
- 保存: Parquet + DuckDB（または Postgres）
- バージョン: Git（プライベート）
- 実験再現: データスナップショットID必須
- ログ: JSONの構造化ログ
...
## 2週間の作業割付
### Week 0（Day 1–5）
1. `CLEAN_ROOM_POLICY.md`、`PROVENANCE.yaml`、研究ノート雛形を作成
2. リポジトリ雛形と実行コマンド体系（`make ingest_spot` 等）を確立
3. Spot取得→保存→QC→日次更新を通す
